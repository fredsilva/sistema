...
public interface PedidoAreasRepository extends BaseRepository<PedidoAreas, Integer> {
    ...
    Integer getLastOrdemParecerFromTipo(@Param("idTipoPedido") Integer idTipoPedido);
    ...
    Integer findIdByParecerFinalAndTipoPedido(@Param("parecerFinal") Boolean parecerFinal,
            @Param("idTipoPedido") Integer idTipoPedido);
    ...
    @Query(value = "SELECT NVL(SUM(pa.quantidade_dias_analise), 0) FROM sefaz_arr.ta_pedido_areas pa"
            + " WHERE pa.id_tipo_pedido = :idTipoPedido AND pa.id_pedido_area <> NVL(:idPedidoArea, 0)",
            nativeQuery = true)
    Integer getTotalQtdDiasAnaliseByTipoAndNotId(@Param("idTipoPedido") Integer idTipoPedido,
