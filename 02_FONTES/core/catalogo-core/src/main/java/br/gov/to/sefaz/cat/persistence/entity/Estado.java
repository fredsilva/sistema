...
/**
 * Entidade referente a tabela SEFAZ_CAT.TA_ESTADO do Banco de Dados.
 *
 * @author <a href="mailto:cristiano.luis@ntconsult.com.br">cristiano.luis</a>
 * @since 02/05/2016 14:45:32
 */
@Entity
@Table(name = "TA_ESTADO", schema = "SEFAZ_CAT")
public class Estado extends AbstractEntity<String> {

    @Id
    @NotNull(message = "#{cat_msg['estados.unidadeFederacao.obrigatorio']}")
    @Size(min = 2, max = 2, message = "#{cat_msg['estados.unidadeFederacao.tamanho']}")
    @Column(name = "UNIDADE_FEDERACAO", nullable = false, length = 2)
    private String unidadeFederacao;

    @Size(max = 40, message = "#{cat_msg['estados.nomeEstado.tamanho']}")
    @Column(name = "NOME_ESTADO", length = 40)
    private String nomeEstado;

    @Max(value = 999, message = "#{cat_msg['estados.aliquotaOrigemEntrada.maximo']}")
    @Min(value = 1, message = "#{cat_msg['estados.aliquotaOrigemEntrada.minimo']}")
    ...
    @Size(max = 10, message = "#{cat_msg['estados.cepGeralUf.tamanho']}")
    @Column(name = "CEP_GERAL_UF", length = 10)
    private String cepGeralUf;

    @Max(value = 9, message = "#{cat_msg['estados.codigoRegiao.maximo']}")
    @Min(value = 1, message = "#{cat_msg['estados.codigoRegiao.minimo']}")
    @Column(name = "CODIGO_REGIAO")
    private Integer codigoRegiao;

    @Max(value = 999, message = "#{cat_msg['estados.cargaTributariaOrigem.maximo']}")
    @Min(value = 1, message = "#{cat_msg['estados.cargaTributariaOrigem.minimo']}")
