 ...
 */
@Component
public class UnidadeOrganizacionalUnidadePaiValidator implements ServiceValidator<Long> {

    private final UnidadeOrganizacionalRepository unidadeOrganizacionalRepository;

    @Autowired
    public UnidadeOrganizacionalUnidadePaiValidator(UnidadeOrganizacionalRepository unidadeOrganizacionalRepository) {
    ...
    public Set<CustomViolation> validate(Long target) {
        ...
        if (unidadeOrganizacionalRepository.existsLockReferencePai(target)) {
            String codigoCadastrado = SourceBundle.getMessage(MessageUtil.SEG,
                    "seg.gestao.unidadesOrgazinacionais.form.falha.unidOrganizacPai");
